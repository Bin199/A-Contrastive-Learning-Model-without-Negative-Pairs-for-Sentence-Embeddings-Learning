...
def informin_cl_loss(feature1, feature2):
...
def cl_forward(cls,
    ...
    # self_contrast = torch.diag(
    #     cls.sim(z1.unsqueeze(1), z2.unsqueeze(0))).sum()

    # z1_norm = F.normalize(z1, dim=-1)
    # z2_norm = F.normalize(z2, dim=-1)

    loss = cls.config.task_alpha * self_contrast + cls.config.task_beta * decorrelation
    # loss += cls.model_args.informin_cl_coefficient * informin_cl_loss(z1_norm, z2_norm)
    # loss += cls.model_args.informin_cl_coefficient * 0.5 * informin_cl_loss(z1_norm, z2_norm)
    # loss += cls.model_args.informin_cl_coefficient * 0.5 * informin_cl_loss(z2_norm, z1_norm)
    # SCD BEGIN: SimCSE legacy code
